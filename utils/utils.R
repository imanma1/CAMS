...
oracle_sc_prob <- function(oracle_mdl, data, t) {
  ...
  # ==========================================================
  # Controlled DR setting with X3-dependent event/censoring
  # heterogeneity
  # ==========================================================

  if (setting == "cov_cens_dr_tail_hetero") {
    ...
    # Event-location function used by the DGP.
    mu_t <- 2 +
      0.5 * X$X2 -
      0.5 * X$X1

    # Censoring heterogeneity is linked to X3,
    # not directly to the protected validity groups.
    prob_early <- ifelse(
      X$X3 > 0,
      0.40,
      0.10
